...
# These process/system functions are now implemented cross-platform, so they
# are parity-tested on every OS. The *values* are environment-specific, so we
# assert by type/shape only (the booleans below are platform-independent):
#   getpid    -> std::process::id()
#   getppid   -> libc::getppid() on Unix, Toolhelp snapshot on Windows
#   cpu_count -> std::thread::available_parallelism()
#   urandom   -> getrandom crate (OS CSPRNG on Linux/macOS/Windows)
#   strerror  -> std::io::Error::from_raw_os_error()
