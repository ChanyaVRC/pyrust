...
impl Compiler {
    ...
    fn build_def_proto(
        ...
        if self.fn_protos.len() >= u32::MAX as usize {
            self.failed = true;
            if self.error_msg.is_none() {
                self.error_msg =
                    Some("too many functions in one scope (max 4294967295)".to_string());
            }
            return None;
        }
        let proto_idx = self.fn_protos.len() as u32;
    ...
    fn build_class_proto(
        ...
        if self.fn_protos.len() >= u32::MAX as usize {
            ...
            if self.error_msg.is_none() {
                self.error_msg =
                    Some("too many classes/functions in one scope (max 4294967295)".to_string());
            }
            return None;
        }
        let proto_idx = self.fn_protos.len() as u32;
