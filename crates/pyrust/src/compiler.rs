    ...
    for stmt in body {
        match stmt {
            ...
            Stmt::Class {
                ...
                //
                // We use `collect_global_names_via_class_chain` here rather than
                // `collect_global_names` (non-recursive) so that a `global x`
                // declaration inside a doubly-nested class body (e.g.
                // `class Outer: class Inner: global x`) is also visible when
                // scanning the outer class body (issue #672).
                //
                // We do NOT use `collect_transitive_global_names` (which also
                // crosses `Stmt::Def` boundaries) to avoid the issue #624
                // regression where a method's `global x` would incorrectly
                // promote the enclosing scope's `x` to a cell var.
