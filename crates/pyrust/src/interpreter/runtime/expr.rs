            ...
            let candidates: Vec<(usize, Value, Value)> = {
                ...
                dict.iter()
                    ...
                    .filter_map(|(i, (k, v))| match k {
                        ...
                        // PyKey::None has Python-level hash 0.  When the Object
                        // key also hashes to 0, check whether __eq__ considers
                        // them equal (issue #906).
                        PyKey::None if *target_hash == 0u64 => {
        ...
        if matches!(key, PyKey::None) {
            let candidates: Vec<(usize, Value, Value)> = {
                ...
                dict.iter()
                    .enumerate()
                    .filter_map(|(i, (k, v))| match k {
                        PyKey::Object { hash: 0, value } => Some((i, value.clone(), v.clone())),
                        _ => None,
                    })
                    .collect()
            };
            ...
            let candidates: Vec<(usize, Value)> = {
                ...
                set.iter()
                    ...
                    .filter_map(|(i, k)| match k {
                        ...
                        PyKey::None if *target_hash == 0u64 => Some((i, Value::none())),
                        _ => None,
                    })
                    .collect()
            };
        ...
        if matches!(key, PyKey::None) {
            let candidates: Vec<(usize, Value)> = {
                ...
                set.iter()
                    .enumerate()
                    .filter_map(|(i, k)| match k {
                        PyKey::Object { hash: 0, value } => Some((i, value.clone())),
                        _ => None,
                    })
                    .collect()
            };
