...
impl Interpreter {
    ...
    fn exec_call_method_kw(
                ...
                self.resolve_method_cached(regs, obj, method, code, call_site_pc)
        {
            ...
            let cached: Option<smallvec::SmallVec<[u32; 4]>> = {
                ...
                match &cache[call_site_pc] {
                    KwCallCacheEntry::Simple { param_binds_ptr, npos: cnpos, slots }
                        if *param_binds_ptr == pbptr && *cnpos as usize == eff_npos =>
                    {
                        Some(slots.clone())
                    }
                    _ => None,
            ...
            let resolved: Option<smallvec::SmallVec<[u32; 4]>> = if f.defaults_override.borrow().is_some() {
                ...
                match Self::kwcall_resolve_simple(&f, eff_npos, &kwnames) {
                    Some(slots) => {
                        code.kwcall_cache.borrow_mut()[call_site_pc] = KwCallCacheEntry::Simple {
                            param_binds_ptr: pbptr,
                            npos: eff_npos as u8,
                            slots: slots.clone(),
                        };
                    ...
                    None => {
                        // Not simple — permanent fallback to the general binder.
                        code.kwcall_cache.borrow_mut()[call_site_pc] = KwCallCacheEntry::Fallback;
                        None
                    }
                }
            } else {
        ...
        let method: String = code
            ...
            .ok_or_else(|| {
                PyError::Runtime(format!("bytecode error: name index {name_idx} out of range"))
            })?
            .as_str()
            .to_string();
        let kwnames = match code.consts.get(kwnames_idx as usize).and_then(|c| c.as_tuple()) {
