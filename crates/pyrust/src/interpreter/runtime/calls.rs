...
fn parse_format_spec(spec: &str, type_name: &str) -> Result<FormatSpec> {
    ...
    let width: usize = if width_end > width_start {
        spec[width_start..width_end].parse::<usize>().map_err(|_| {
    ...
    // precision
    let precision = if matches!(chars.peek(), Some(&(_, '.'))) {
        chars.next();
...
impl Interpreter {
    ...
    pub(crate) fn format_value_default(&mut self, value: &Value) -> Result<Value> {
        ...
        // Issue #2771: a bare `{cls}` field is `str(cls)`, which dispatches the
        // metaclass `__str__`/`__repr__` when overridden.  Returns `None` for an
        // ordinary class, so plain classes keep the fast `apply_format_spec`
        // path below.
        if let ValueKind::PyClass(cls_rc) = value.kind() {
            let cls_rc = Rc::clone(cls_rc);
            if let Some(res) =
